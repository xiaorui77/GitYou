...
public class AuthController {
     ...
     * */
    @PostMapping("login")
    public ResponseEntity<ResponseResult<UserInfo>> login(String username, String password,
                                                          HttpServletRequest request, HttpServletResponse response) {
        ResponseResult<User> result = authService.authentication(username, password);
        ...
        if (result.getCode() == 200) {
            ...
            return ResponseEntity.ok(new ResponseResult<>(result.getCode(), "登录成功", info));
        } else
            return ResponseEntity.status(result.getCode()).body(new ResponseResult<>(result.getCode(), result.getMsg()));
    }
     ...
     * */
    @PostMapping("logout")
    public ResponseEntity<ResponseResult<UserInfo>> logout(String username, String password,
                                                           HttpServletRequest request, HttpServletResponse response) {
     ...
     * */
    @PostMapping("verify")
    public ResponseEntity<ResponseResult<UserInfo>> verify(@CookieValue("user-identity") String token,
                                                           HttpServletRequest request, HttpServletResponse response) {
        try {
            UserInfo userInfo = JwtUtils.getInfoFromToken(token, authProperties.getPublicKey());
            return ResponseEntity.ok(new ResponseResult<>(200, "验证通过", userInfo));
        } catch (Exception e) {
            e.printStackTrace();
